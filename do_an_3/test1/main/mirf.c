...
void Nrf24_setRADDR_P1(NRF24_t * dev, uint8_t * adr)  // Sets the receiving device address
{
	Nrf24_ceLow(dev);
...
void Nrf24_setRADDR_P2(NRF24_t * dev, uint8_t * adr)  // Sets the receiving device address
{
	Nrf24_ceLow(dev);
	Nrf24_writeRegister(dev, RX_ADDR_P2, adr, mirf_ADDR_LEN);
	Nrf24_ceHi(dev);
}
