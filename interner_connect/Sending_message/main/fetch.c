...
void fetch(char *url, struct FetchParms *fetchParms)
{
    ...
    {
        esp_http_client_set_header(client, fetchParms->header[i].key, fetchParms->header[i].value);
    }
    if(fetchParms->body != NULL)
    {
        esp_http_client_set_post_field(client, fetchParms->body, sizeof(fetchParms->body));
    }
